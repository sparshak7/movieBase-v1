...
const FavoritesCard = ({ type, item, setFavorites }) => {
  const { removeFavorite } = useFireStore();
  const { user } = useAuth();
  const imgUrl = "https://image.tmdb.org/t/p/original/";

  const handleRemoveClick = (e) => {
    e.preventDefault();
    removeFavorite(user?.uid, item?.id).then(() => {
      setFavorites((prev) => prev.filter((idx) => idx.id !== item.id));
    });
  };
